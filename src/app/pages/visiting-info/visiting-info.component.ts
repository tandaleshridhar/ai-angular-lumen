...
export class VisitingInfoComponent implements OnInit {
  ...
  add21Days(){
    this.hitDate = moment(moment().add(21, 'days')).format('YYYY MM DD');
  }

  add75Days(){
    this.pdDate = moment(moment().add(75, 'days')).format('YYYY MM DD');
  }

  add9Month(){
    this.deliveryDate = moment(moment().add(279, 'days')).format('YYYY MM DD');
  }
